...
use crate::{
    info::Info, Cache, Distance, DistanceCmp, Embedding, EmbeddingProvider, LocalCache,
    LocalCacheFactory, NearestNeighbors,
};
    ...
    T: 'a,
{
    ...
    fn get_dist<I, L>(&self, provider: &'a E, cache: &mut L, info: &mut I) -> DistanceCmp
    where
        I: Info,
        L: LocalCache<'a, D, T>,
    {
    ...
    fn get_dist_min<I, L>(&self, provider: &'a E, cache: &mut L, info: &mut I) -> DistanceCmp
    where
        I: Info,
        L: LocalCache<'a, D, T>,
    {
    ...
    fn get_child_dist_max(child: &Child<'a, E, D, T>) -> DistanceCmp {
        child
            .center_dist
        ...
        L: LocalCache<'a, D, T>,
    {
        ...
        if is_outer {
            ...
            let mut inners: Vec<(&Node<E, D, T>, DistanceCmp)> = self
                .children
                .iter()
...
{
    provider: &'a E,
    root: Option<Node<'a, E, D, T>>,
    high_ix: usize,
}

impl<'a, E, D, T> Fann<'a, E, D, T>
where
    E: EmbeddingProvider<'a, D, T>,
    D: Distance<T> + Copy,
    T: 'a,
{
    ...
    where
        ...
        I: Info,
    {
        cache.cached_distance(embed_a, embed_b, self.provider.distance(), info)
    }

    fn centroid<C, I>(&self, all_ixs: &Vec<usize>, cache: &mut C, info: &mut I) -> usize
    where
        C: Cache,
        I: Info,
    {
                    ...
                    let cur_dist: DistanceCmp =
                        all_ixs.iter().fold(DistanceCmp::zero(), |res, &oix| {
                            if oix == ix || res > best_dist {
                                ...
                                res.combine(
                                    &self.get_dist(&embed, &oembed, cache, info),
                                    |cur, dist| cur + dist,
                                )
                            }
                        });
        ...
        I: Info,
    {
        ...
        loop {
            ...
            all_ixs
                ...
                .for_each(|&ix| {
                    ...
                    let (_, best) = res
                        .iter_mut()
                        .min_by(|(a, _), (b, _)| {
                            let dist_a = self.get_dist(&embed, &provider.get(*a), cache, info);
                            let dist_b = self.get_dist(&embed, &provider.get(*b), cache, info);
                            dist_a.cmp(&dist_b)
                        })
                        .unwrap();
                    best.push(ix);
            ...
            let new_cs: Vec<usize> = res
                .iter()
                .map(|(_, assignments)| self.centroid(assignments, cache, info))
                .collect();
        ...
        I: Info,
    {
        ...
        let root_ix = self.centroid(&all_ixs, cache, info);

        fn remove(ixs: &mut Vec<usize>, index: usize) {
            ixs.retain(|&ix| ix != index);
        }

        fn build_level<'a, E, D, C, T, I>(
            tree: &Fann<'a, E, D, T>,
            cache: &mut C,
            info: &mut I,
            cur_root_ix: usize,
        ...
        where
            ...
            T: 'a,
        {
            let mut node: Node<'a, E, D, T> = Node::new(cur_root_ix);
            ...
            if num_k == 1 || cur_all_ixs.len() <= num_k {
                cur_all_ixs.iter().for_each(|&ix| {
                    let mut cnode: Node<'a, E, D, T> = Node::new(ix);
                    cnode.compute_radius();
                    node.add_child(cnode, tree.provider, cache, info);
                });
            } else {
                ...
                tree.kmedoid(cur_all_ixs, init_centroids, num_k, cache, info)
                    .into_iter()
                    .for_each(|(centroid_ix, mut assignments)| {
                        remove(&mut assignments, centroid_ix);
                        let child_node = build_level(
                            tree,
                            cache,
                            info,
                            centroid_ix,
                        ...
                        node.add_child(child_node, tree.provider, cache, info);
                    });
            }
            node.compute_radius();
            node
...
impl<'a, F, E, D, L, T> NearestNeighbors<'a, F, D, L, T> for Fann<'a, E, D, T>
where
    ...
    T: 'a,
{
        ...
        I: Info,
    {
        ...
        self.root
            .as_ref()
            .unwrap()
            .get_closest(&mut res, count, self.provider, &mut cache, info);
